...
export interface CoachSearchResult {
    ...
    apRank?: number | null;
    /**
     * Total compensation in cents (included based on authorization). On every season this is the latest known salary for the row's assignment (WINAD-10478, WINAD-10482): the season's own record when it carries a usable total, otherwise the most recent usable record from the same coach, school, sport and position types, however many seasons back, as long as that job was held in every season between. The comp filters, the compensation sort and comp_stats read the same value, which is as fresh as the last search-view refresh.
     * @type {number}
     * @memberof CoachSearchResult
     */
    ...
    /**
     * True when the figures were carried forward from an earlier season of the same assignment because the searched season has no usable record (included based on authorization).
     * @type {boolean}
     * @memberof CoachSearchResult
     */
