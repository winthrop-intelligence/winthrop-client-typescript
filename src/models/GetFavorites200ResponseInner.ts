...
export interface GetFavorites200ResponseInner {
    ...
    /**
     * Absolute URL to the school's small logo variant for FilTeam and Deal favorites (only when detailed=1). Null when the school has no logo, in which case the client falls back to initials. Absolute, unlike avatar_url, matching the other logo endpoints in this API.
     * @type {string}
     * @memberof GetFavorites200ResponseInner
     */
    schoolLogoUrl?: string | null;
    /**
     * ID of the deal's contract for Deal favorites (only when detailed=1). The contract page is the only page a single deal has, so this is what the row links to; null when the deal has no contract, in which case the client links to the school's deal list instead.
     * @type {number}
     * @memberof GetFavorites200ResponseInner
     */
