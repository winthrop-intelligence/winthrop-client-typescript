...
| [**getGameContractApplyCapabilities**](GameContractsApi.md#getgamecontractapplycapabilities) | **GET** /api/v1/game_contracts/apply_capabilities |  |
...
## getGameContractApplyCapabilities

> GameContractApplyCapabilities getGameContractApplyCapabilities()



Report which apply plan schema versions and operations this deployment can actually execute, and whether retained contract history is available. Fails closed - v2 operations are withheld entirely unless the retained-history table exists, so an unmigrated deployment cannot invite traffic it would then reject.
...
import type { GetGameContractApplyCapabilitiesRequest } from '@winthrop-intelligence/winthrop-client-typescript';
...
async function example() {
  ...
  try {
    const data = await api.getGameContractApplyCapabilities();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
[**GameContractApplyCapabilities**](GameContractApplyCapabilities.md)
...
| Status code | Description | Response headers |
|-------------|-------------|------------------|
| **200** | The capabilities this deployment advertises |  -  |
| **401** | Unauthorized |  -  |
| **403** | Forbidden |  -  |
