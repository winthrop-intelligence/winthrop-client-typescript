...
| [**createGamePost**](DefaultApi.md#creategamepostoperation) | **POST** /api/v1/game_posts |  |
...
> GamePost createGamePost(createGamePostRequest)



Create a game post. It lands on the school named by &#x60;school_id&#x60;, and on the current user\&#39;s own school when that is omitted.  WINAD-10272: a school-unscoped grant (scheduling staff loading an emailed schedule, or a super admin) may name any school; a school-scoped user naming a school other than their own is refused with 403, and an unknown id is 404.
...
import type { CreateGamePostOperationRequest } from '@winthrop-intelligence/winthrop-client-typescript';
...
async function example() {
  ...
  const body = {
    ...
    createGamePostRequest: ...,
  } satisfies CreateGamePostOperationRequest;
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **createGamePostRequest** | [CreateGamePostRequest](CreateGamePostRequest.md) |  | |
...
| **401** | Unauthorized |  -  |
| **403** | Forbidden — the caller may not create game posts for the named school |  -  |
| **404** | Not Found — school_id names no school |  -  |
...
Create a private /schedules grid marker (WINAD-9646). This is NOT a public Games Wanted post — it never reaches the /game_posts feed, the school-detail badge, or the digest email.  WINAD-10272: the marker lands on the school named by &#x60;school_id&#x60;, and on the current user\&#39;s own school when that is omitted. A school-unscoped grant (scheduling staff loading an emailed schedule, or a super admin) may name any school; a school-scoped user naming a school other than their own is refused with 403, and an unknown id is 404.
...
| **422** | Validation error (no school on the account, or invalid attributes) |  -  |
| **401** | Unauthorized |  -  |
| **403** | Forbidden — the caller may not create schedule intents for the named school |  -  |
| **404** | Not Found — school_id names no school |  -  |
...
> PositionTypeCollection getPositionTypes(page, perPage, q)
...
import type { GetPositionTypesRequest } from '@winthrop-intelligence/winthrop-client-typescript';
...
async function example() {
  ...
  const body = {
    ...
    // object | Ransack query. A value whose key ends in `_in` is split on commas into a list, so a multi-value predicate travels as one parameter — e.g. `q[primary_conference_division_name_in]=DI,DII`. A blank value yields an empty list, which Ransack drops: the predicate then does not filter at all, rather than matching nothing. (optional)
    q: Object,
  } satisfies GetPositionTypesRequest;

  try {
    const data = await api.getPositionTypes(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| Status code | Description | Response headers |
|-------------|-------------|------------------|
| **200** | Position types were found |  -  |
| **401** | Unauthorized |  -  |
| **403** | Forbidden. The credentials are authenticated but lack read access to the position-type catalog (only superadmin / trusted-service credentials are granted access). |  -  |
