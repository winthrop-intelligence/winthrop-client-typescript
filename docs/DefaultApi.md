...
async function example() {
  ...
  const body = {
    ...
    // object | Ransack query (optional)
    q: Object,
    // string | When supplied, each returned school includes a `rank` field with its latest sport-appropriate ranking (NET/AP/RPI) for that sport. (optional)
    sportName: sportName_example,
...
| **perPage** | `number` | number of results per page. | [Optional] [Defaults to `20`] |
| **q** | `object` | Ransack query | [Optional] [Defaults to `undefined`] |
| **sportName** | `string` | When supplied, each returned school includes a &#x60;rank&#x60; field with its latest sport-appropriate ranking (NET/AP/RPI) for that sport. | [Optional] [Defaults to `undefined`] |
